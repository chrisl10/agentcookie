...
- Install method (updated stdin-safe equivalent): `read -rsp 'Pairing code: ' AGENTCOOKIE_PAIR_CODE; printf '\n'; printf '%s\n' "$AGENTCOOKIE_PAIR_CODE" | ssh matts-mac-mini ./install-beta.sh --as sink --peer MacBook-Pro-8.local --pair-url <url> --tarball <local> --code-stdin; unset AGENTCOOKIE_PAIR_CODE`.
